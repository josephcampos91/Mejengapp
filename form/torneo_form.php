...
<div class="table-responsive">
    ...
    while($row = $resul->fetch_assoc()) {
		...
		if ($resul_equipo_jugador->num_rows > 0) {
			...
			<tr>
				<td>
					<?php  echo strtoupper($row1["junom"]);  ?>
				</td>
				<td>
					<?php  echo strtoupper($row1["quinom"]);  ?>
				</td>
			</tr>
			<?php }
		}else{
			...
			<tr>
				<td colspan="2">
					<form action="../class/cls_gestor.php" class="form-signin" method="post">
						<input type="text" class="form-control" name="gestor" hidden value="<?= $value_torneo_ini?>" required />
						<input type="text" class="form-control" name="edit_id" hidden value="<?= $fk_torneo_id ?>" required />
						<button class="btn btn-lg btn-<?= $btn_style ?> btn-block" type="submit">
							<?= $btn_torneo_ini ?>
						</button>
					</form>
				</td>
